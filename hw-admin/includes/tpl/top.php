		...
		<div class="header">
			...
			<ul class="nav user-menu">
				...
				<li class="nav-item dropdown has-arrow flag-nav">
					<?php
					// Check if the 'langue' key exists in the session before using it
					if (isset($_SESSION['langue'])) {
						$currentlang = langue::findByCode($_SESSION['langue']);
					} else {
						// Fallback to a default language if 'langue' is not set
						$currentlang = langue::getDefaultLanguage(); // Ensure this method retrieves a default langue object
					}
					?>
					...
					<div class="dropdown-menu dropdown-menu-right">
						...
						foreach ($modules as $module) {
							...
							if ($_SESSION['user']->hasDroit('view', $module->getIdModule())) { ?>
								<a class="dropdown-item" href="index.php?option=<?php echo $module->getIdModule(); ?>">
									<i class="mr-2 fa fa-<?= $module->getIcon(); ?>"></i> <?php echo $module->getNom(); ?>
								</a>
						<?php }
						} ?>
					</div>
				...
				<li class="nav-item dropdown has-arrow flag-nav">
					<?php
					// Check if 'langue' is set and assign the current language
					if (isset($_SESSION['langue'])) {
						$currentlang = langue::findByCode($_SESSION['langue']);
					} else {
						$currentlang = langue::getDefaultLanguage(); // Fallback default language
					}
					?>
					<a class="nav-link dropdown-toggle" data-toggle="dropdown" href="#" role="button">
						<img src="assets/img/flags/<?php echo isset($currentlang) ? $currentlang->getCode() : 'default_code'; ?>.png" alt="" height="20">
						<span><?php echo isset($currentlang) ? $currentlang->getNom() : 'Default Name'; ?></span>
					</a>
					<div class="dropdown-menu dropdown-menu-right">
