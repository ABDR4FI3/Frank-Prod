    ...
    <div class="row">
        ...
        <?php if (isset($photo) && $photo->getPhoto()) { ?>
            <div class="col-md-2 form-group mt-4">
                <img src="images/galerie/<?= $photo->getPhoto(); ?>" alt="" class="img-thumbnail" />
            </div>
        <?php } ?>



        <input type="hidden" name="id_galerie" value="<?= $galerie->getId() ?>" />
        <?php if (isset($photo)) { ?>
        ...
        $('form#galeriePhotoForm').ajaxForm({
            ...
            success: function(theResponse) {
                ...
                if (parseInt(theResponse) === 1) {
                    $('#galeriePhotoForm .msgbox').html('<div class="alert alert-success alert-dismissable"><i class="icon-check-sign"></i> <strong>Succès</strong> ' + msgsucces + '</div>').slideDown();
                    setTimeout(function() {

                        <?php $loc = "index.php?option=com_galerie&task=galeriephotos&id=" . $_GET['id']; ?>
                        document.location = "<?= $loc ?>";

                    }, 1500);
                } else if (parseInt(theResponse) === 0) {
                    $('#galeriePhotoForm .msgbox').html('<div class="alert alert-warning alert-dismissable"><i class="icon-remove-sign"></i> <strong>Attention!</strong> ' + msgvide + '</div>').slideDown();
                } else {
                    $('#galeriePhotoForm .msgbox').html('<div class="alert alert-danger alert-dismissable"><i class="icon-remove-sign"></i> <strong>Erreur!</strong> ' + msgfaild + '</div>').slideDown();
                }
            }
        });
    })
