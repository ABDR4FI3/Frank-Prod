...
class menu_item
{
    ...
    public function edit()
    {
        ...
        if (!$db->query($SQLupdate)) {
            $SQLselect = sprintf("SELECT * FROM " . static::$table2 . " WHERE id_menu_item = %s AND langue = %s", 
            GetSQLValueString($this->getId(), "int"), 
            GetSQLValueString($this->getLangue(), "text"));
            //echo $SQLselect;
    ...
    public static function find($id, $lang = 'fr')
    {
        ...
        $SQLselect = sprintf(
            $SQLselect = "SELECT A.id as ID, A.*, B.* FROM " . static::$table . " A
    ...
    public static function findAll($id, $langue = 'fr', $parent_id = 0, $claus = null, $ordre = false)
    {
        global $db;
        $items = array();
        $menu_item = new menu_item();
        ...
        $SQLselect = sprintf(
            "SELECT A.id as ID, A.*, B.* FROM " . static::$table . " A LEFT JOIN " . static::$table2 . " B ON A.id = B.id_menu_item AND langue = %s 
            WHERE A.id_menu = %s AND A.parent_id = %s",
